...
export class GrassEngine {
  ...
  constructor(container: HTMLElement, config: GrassConfig) {
    ...
    this.scene.fog = new THREE.Fog('#0a0a0a', 5, 30);
    ...
    const groundMat = new THREE.MeshStandardMaterial({ color: '#0b0b0b', roughness: 0.9 });
  ...
  public updateConfig(config: GrassConfig, prevConfig?: GrassConfig) {
    ...
    // Handle Color string to Three.Color conversion safely
    ...
    // Heavy updates (Geometry regeneration)
    if (prevConfig) {
